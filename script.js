...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const projectDetails = {
    ...
    crm_lead: {
      title: "CRM System for Lead Generation",
      subtitle: "7-Stage AI-Powered Lead Qualification Pipeline",
      icon: "bi-funnel-fill",
      problem: "Sales and recruitment teams manually sift through hundreds of emails to identify and qualify R&D leadership opportunities — a slow, error-prone process that causes missed leads and delays in the entire sales pipeline.",
      solution: "A fully automated 7-stage pipeline: (1) Gmail IMAP ingestion with subject-line normalization and de-duplication — duplicates flagged SKIPPED_DUPLICATE_EMAIL; (2) Groq LLM (Llama-3.1-8b-instant) extracts R&D/Innovation leadership roles targeting Manufacturing, Auto, Aerospace, Pharma, MedTech & GCC sectors — structured JSON output; (3) Serper API searches and prioritizes job URLs by strict rank: LinkedIn → Naukri → Indeed → BeBee → SimplyHired → Official site; (4) Playwright scrapes full job descriptions and caches company employee-count for 7 days to avoid repeat lookups; (5) Mistral LLM scores each lead and applies a Strategy boost (+15) or Pricing penalty (−20) based on role fit; (6) Threshold engine qualifies leads only if employee count ≥ 500 (or score ≥ 80 when data is missing) AND final score ≥ 50; (7) Qualified leads auto-appended to Google Sheets and saved locally — rejected leads logged with detailed reasons for full traceability.",
      techs: ["Node.js", "Groq API (Llama-3.1)", "Playwright", "Serper API", "Mistral LLM", "Google Sheets API", "IMAP"],
    ...
    crm: {
      ...
      problem: "Sales and recruitment teams manually sift through hundreds of emails to identify and qualify R&D leadership opportunities — a slow, error-prone process that causes missed leads and delays in the entire sales pipeline.",
      solution: "A fully automated 7-stage pipeline: (1) Gmail IMAP ingestion with subject-line normalization and de-duplication — duplicates flagged SKIPPED_DUPLICATE_EMAIL; (2) Groq LLM (Llama-3.1-8b-instant) extracts R&D/Innovation leadership roles targeting Manufacturing, Auto, Aerospace, Pharma, MedTech & GCC sectors — structured JSON output; (3) Serper API searches and prioritizes job URLs: LinkedIn → Naukri → Indeed → BeBee → SimplyHired; (4) Playwright scrapes full job descriptions and caches company employee data for 7 days; (5) Mistral LLM scores leads with Strategy boost (+15) or Pricing penalty (−20); (6) Threshold engine: employee count ≥ 500 AND final score ≥ 50; (7) Qualified leads auto-logged to Google Sheets with full rejection traceability.",
      techs: ["Node.js", "Groq API (Llama-3.1)", "Playwright", "Serper API", "Mistral LLM", "Google Sheets API"],
      link: "https://github.com/vimalRaj45/crm-finalv1",
    },
    drkankas: {
    ...
    smb: {
      ...
      techs: ["React.js", "CSS Animations", "Scroll Effects"],
